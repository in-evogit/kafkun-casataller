...
export interface CoverflowCarouselProps {
  ...
  cardWidth?: string;
  /**
   * Proporcion ancho/alto de la tarjeta. 1 = cuadrada (el original).
   *
   * ADAPTACION para Kafkun: las obras se fotografian VERTICALES (3:4) porque asi
   * se fotografia una prenda colgada. Con tarjetas cuadradas el recorte se come un
   * cuarto de cada pieza — justo lo que lib/media.ts existe para evitar.
   */
  cardAspect?: number;
...
export function CoverflowCarousel({
  ...
  const [menosMovimiento, setMenosMovimiento] = React.useState(false);

  // Con "reducir movimiento" no se anima el asentamiento: se salta directo a la
  // tarjeta. El carrusel sigue siendo usable, solo deja de moverse.
  React.useEffect(() => {
  ...
  const alto = `calc(var(--cf-card) / ${cardAspect})`;
      ...
      <div className="relative">
        ...
        >
          <div
            ...
            style={{ height: alto, transformStyle: "preserve-3d" }}
          >
            {slides.map((slide, index) => (
              <div
                ...
                style={{ width: "var(--cf-card)", height: alto }}
              >
                {/* next/image en vez de <img>: sirve AVIF y WebP y evita bajar
                    1400px cuando la tarjeta mide 260. */}
                <Image
                  src={slide.src}
                  alt={slide.alt}
                  fill
